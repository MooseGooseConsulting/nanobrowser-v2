...
under `tests/`, `src/` and `entrypoints/` (70 files, 1521 tests) and then
...
| --- | --- | --- | --- | --- |
| **R-01** Acts on the tab the user already has open | done | `src/runtime/runManager.ts` (`chromeTabsPort`, `refuseReason`), `src/page/driver.ts` | `src/runtime/runManager.test.ts`: "acts on the active tab of the last focused window and names it in run.started (R-01)", "refuses a chrome:// tab with a run.ended error rather than starting", "refuses the side panel itself" (these prove `RunManager` forwards whatever a fake `TabsPort` returns); `src/runtime/runManager.edge.test.ts`: "prefers the active tab of the last focused normal window", "falls back to chrome.tabs.query when getLastFocused throws (e.g. every window minimised)", "falls back to chrome.tabs.query when the focused window has no active tab" (the real `chromeTabsPort()` selection logic itself, against a mocked `chrome.windows`/`chrome.tabs`, previously untested by anything) | No tab is ever created; the active tab of the last focused normal window is the target, and browser/extension pages are refused with a clear `run.ended{error}`. |
...
| **R-09** The agent can execute userscripts live | done | `src/userscripts/runner.ts`, `src/runtime/pageTools.ts` (`runUserscript`, `listUserscripts` tools), `entrypoints/background.ts` | `src/userscripts/runner.test.ts`: "injects into the USER_SCRIPT world, immediately, in the named tab", "returns the script value and the captured console lines"; `src/runtime/pageTools.test.ts`: "emits userscript console lines into the run log (R-07/R-09)" | The Follower's `run_userscript` tool runs a stored script against the run's tab; its console lines become `userscript.output` events, and now also come back to the model in the tool reply, which is what makes R-10's loop observable. `list_userscripts` lets it discover ids rather than only using ones it was handed. |
| **R-10** The agent can debug userscripts live | done | `src/userscripts/authoring.ts` (the agent's write path and its rails), `src/userscripts/debug.ts` (`DebugSession`, the panel's loop), `src/runtime/pageTools.ts` (`write_userscript`), `src/ui/sections/UserscriptsSection.tsx` | `src/userscripts/authoring.test.ts`: "refuses to overwrite a script the user wrote", "refuses a public-suffix wildcard, which used to read as a concrete host", "refuses a script that would run on every site the user visits", "refuses to aim agent-written code at anything but the web", "does not clobber a user save that lands between the check and the write", "stops creating once the catalog holds MAX_AGENT_SCRIPTS of the agent's own", "revises a script it wrote before, in place, keeping the id"; `src/runtime/runManager.test.ts`: "shows the agent its own scripts and the user's that already apply to the tab"; `src/userscripts/index.test.ts`: "makes an agent-written script the user's once they save it themselves"; `src/agent/graph.test.ts`: "can write a script, run the id it gets back, and see what the script returned"; `src/runtime/pageTools.test.ts`: "gives the model what the script logged, not just what it returned", "carries the console into the failure too, where it is needed most", "fails the step with the refusal reasons verbatim, so the retry can be correct"; `src/ui/sections/UserscriptsSection.test.tsx`: "marks a script the agent wrote and leaves the user's own unmarked"; `src/userscripts/debug.test.ts`: "re-runs edited code in place and replaces the previous result", "exposes the last run as run-log events, with the error line mapped back"; `src/ui/sections/UserscriptsSection.test.tsx`: "sends the live editor code on Run, without saving first (O-03)"; `src/userscripts/debug.edge.test.ts`: "keeps the result of the call started last, even when it resolves first", "attributes an in-flight run to the script it was started against, not whatever select() made current" (a real stale-result race under overlapping `rerun()` calls, found by this suite and fixed in `debug.ts` — see docs/test-review.md) | **O-03 answered 2026-09-04: the agent writes and iterates scripts, and breakpoints are dropped to keep R-02.** The agent now has the whole loop — `write_userscript` authors or revises the code, `run_userscript` runs it and returns the value, the console lines and, on failure, the error with the line and column mapped back onto the code it wrote. The panel keeps its own human-driven loop unchanged. Four rails sit on the agent's write path, because past that point model-written code executes in the user's real logged-in browser: every host must be written out in full (no wildcard host at all — an adversarial review broke an earlier "not a bare `*`" rule with `*://*.com/*`, which Chrome compiles to a regex matching every `.com` site), only http/https may be targeted, a script the *user* wrote can never be overwritten (re-checked inside the write, against the read it is built from, so a panel save landing mid-flight is not clobbered), and at most `MAX_AGENT_SCRIPTS` may accumulate. `list_userscripts` shows the agent its own scripts plus the user's that already apply to the run's tab — not the whole catalog, which combined with `navigate` into "go to this script's host and run it". Agent-authored scripts are stamped `author: 'agent'` and shown as such in the panel list. Not built and not planned: breakpoints and stepping, which would need `chrome.debugger`'s `Debugger` domain and its permanent infobar. Not yet exercised on real Chrome — see A-01. |
| **R-11** Models chosen in the panel, Leader and Follower separately, validated readiness | done | `src/ui/components/ModelSelect.tsx`, `src/ui/components/Combobox.tsx`, `src/ui/state/models.ts`, `src/ui/state/modelFilter.ts`, `src/storage/config.ts`, `src/runtime/worker.ts` (`models.list`, `readiness.get`), `src/runtime/runManager.ts`, `src/host/native.ts` (`mapKiloModel`, merged `listModels`), `src/agent/models.ts` (`createChatModel` source routing), `src/messaging/contract.ts` (`ModelInfo.source`/`mayTrainOnYourPrompts`) | `src/runtime/worker.test.ts`: "answers from the host and then from the cache for ten minutes", "maps a host-connection failure to an actionable reason", "passes a validated readiness through unchanged (R-11)"; `src/runtime/runManager.test.ts`: "builds the leader and the follower models separately (R-11/C-07)", "threads each role's stored source through to createModel, independently of the other role"; `src/ui/components/ModelSelect.test.tsx`: "keeps Leader and Follower selections independent", "shows the stored model id even before the catalog loads", "flags a stored id that is not in a loaded catalog, distinctly from still-loading", "shows which source each model came from as a badge", "keeps the same id from two sources as two distinct, independently selectable options"; `src/ui/sections/SetupSection.test.tsx`: "defaults on and hides paid models from the picker", "reveals paid models once turned off", "warns when a paid model is picked", "never keeps an existing paid selection out of view: it still resolves, just is not offered as a new pick", "filters the picker by source, defaulting to showing every source", "persists both the model id and its source when a Kilo model is picked", "never writes a source for an OpenRouter pick, keeping old stored configs byte-identical"; `src/ui/state/modelFilter.test.tsx`: "defaults on, per the enforced norm", "defaults to \"all\", so adding Kilo never hides a model that was visible before"; `src/host/native.test.ts`: "tags each entry by source and never drops the Kilo-only variant of a shared id", "still returns the surviving source's models when the other failed, and logs which one"; `src/agent/models.test.ts`: "routes a Kilo-sourced model to the Kilo base URL", "never encodes the source into the model id sent to the provider" | Readiness is the host's validated `GET /key` answer, never assumed; config is saved to and restored from `local:config`. Kilo AI Gateway is the second model source (alongside OpenRouter): the panel picks a model + its source, `Config.leaderModelSource`/`followerModelSource` are optional so a config stored before Kilo existed still routes to OpenRouter unchanged, and the same model id can appear from both sources as two independently selectable entries (e.g. `meta/muse-spark-1.3-contributor`, which 404s on OpenRouter under the `data_collection:deny` default but works on Kilo). |
| **R-12** Credentials never exposed by the extension | done | `src/host/fetch.ts` (allow-listed origins: OpenRouter + Kilo), `src/host/redact.ts`, `host/src/secrets.ts`, `host/src/llm.ts` | `src/host/fetch.test.ts`: "never forwards a client-supplied Authorization header", "rejects an off-origin URL locally, without sending a message", "allows the Kilo AI Gateway prefix alongside OpenRouter", "still refuses an arbitrary host that merely resembles a known prefix", "never injects the OpenRouter-shaped provider block into a Kilo request"; `src/host/fetch.edge.test.ts`: "never forwards a client-supplied Authorization header from a Request", "rejects an off-origin Request locally, without sending a message" (the same guarantees, for the `Request`-instance call shape `fetch.test.ts` never tried); `src/host/redact.test.ts`: "strips an OpenRouter key wherever it appears, nested in unknown args"; `src/host/redact.edge.test.ts`: "strips a dotted key whole, not just up to the first dot", "redacts a lower-cased \"bearer\" header value" (a real regex-drift gap against the host's own `KEY_SHAPES`, and a case-sensitivity gap, both found and fixed — see docs/test-review.md); `host/test/dispatcher.test.ts`: "reports ready when GET /key succeeds, and never sends the key to the panel"; `host/test/secrets.edge.test.ts` (the real `DopplerSecretProvider`, previously tested only via its fake); `host/test/llm.test.ts`: "never attaches a key to an origin the host does not recognise" | The extension holds no key and adds no `Authorization`; the host attaches it, choosing OpenRouter's or Kilo's key by the request's own URL origin. Run-log events are redacted before they leave the extension. |
| **R-13** Escalatable input fidelity, default in-page | done | `src/input/*`, `src/runtime/pageTools.ts` (`EscalatableInput`) | `src/runtime/pageTools.test.ts`: "routes to the in-page tier by ref when fidelity is in-page", "routes to the debugger tier by viewport point when fidelity is escalated", "attaches the debugger tier once for the whole run and detaches at the end", "falls back to the in-page tier for the rest of the run when the user detaches"; `tests/input/debugger.test.ts`: "refuses a non-Input command if one were ever attempted (defensive guard)" (the actual refusal proof; the file's "never sends Runtime/Page/DOM/Emulation commands across a mixed run" is proof-by-omission on its own — see docs/test-review.md); `src/runtime/runManager.edge.test.ts`: "detaches after a run finishes normally", "detaches even when the run ends via abort", "detaches even when the underlying run handle rejects outright" (R-13 hygiene actually firing at the RunManager level, not just inside EscalatableInput in isolation) | Escalation is the user's explicit toggle in the panel, never the standing mode. Attach happens once per run and detach once at `run.ended`; a user-cancelled banner is a one-way fallback to in-page, logged as `input.fidelity{attached:false}`. Never exercised against a real Chrome (see the e2e row). |

## Requirements to investigate

| Item | Status | Implementing file(s) | Proving test(s) | Notes |
| --- | --- | --- | --- | --- |
| **I-01** Satisfy R-13 without giving up R-02 | partial | `docs/research/trusted-input-and-stealth.md` §§1–3, 6; `docs/research/bot-detection-research.md` §D.3 + ranked leaks 6–7; `docs/research/stealth-harness-landscape.md` §§6–7; built as `src/input/debugger.ts` + `EscalatableInput` | `tests/input/debugger.test.ts` (attach-once, `Input`-domain-only, onDetach semantics); `src/runtime/pageTools.test.ts` (escalation + fallback); `tests/input/debugger.edge.test.ts`: "sends zero commands and adds exactly one onDetach listener" (isolates the "attach() alone sends no CDP command" claim, previously only true by omission from other tests' assertions) | All three named unknowns are answered **qualitatively**: the banner cannot be suppressed by any shippable means (`--silent-debugger-extension-api` / policy install only); per-action attach is decisively the wrong pattern (`kAutoCloseDelay = 5s`, browser-wide infobar); attach itself issues no CDP command. Missing: nothing is **measured** — attach latency and the infobar's viewport delta are source-derived and flagged unmeasured, and no evidence exists that any vendor detects attachment. The banner-free alternative the docs recommend (Wayland `zwlr_virtual_pointer_v1` via the native host, `host/src/input/`) is a stub that has never been executed. |
...
| **I-03** Whether R-09/R-10 can route around R-13 via the page's own APIs | done | `docs/research/i03-userscript-page-access.md`; probe in `src/userscripts/i03.ts`, bundled into the catalog as `i03-page-access` in `src/userscripts/examples.ts` | `src/userscripts/i03.edge.test.ts`: "reports all four fields false when there is truly no chrome global", "reports each field independently based on which sub-objects are present", "makes no network request at all when sameOriginFetch is omitted", "makes exactly one same-origin GET when sameOriginFetch is true, and reports its outcome" (the original `i03.test.ts` "reports what the world can reach on the page" and "gates the one network request behind an explicit opt-in" asserted `expect.any(Boolean)` and a source-string `.toContain`, respectively — neither actually proved the claim; see docs/test-review.md) | Answered in both directions with running code: **yes** for same-origin credentialed `fetch` (HttpOnly cookies ride along without being exposed), DOM-derived CSRF tokens and CSP-exempt injection; **no** for the page's own heap/globals, cross-origin without CORS, HttpOnly values, trusted input and breakpoints. The probe was exported and tested but reachable from nowhere — no panel button and no agent tool — so the one instrument that can characterise a new site could not actually be run on one; it is now a bundled catalog entry (`src/userscripts/examples.test.ts`: "runs from the catalog entry and reports what the world reaches"), so both the panel and `run_userscript` can run it. Caveat worth settling: `docs/research/stealth-harness-landscape.md` §1 advises against `chrome.userScripts` (Chrome 138+ per-extension toggle, off by default) while I-03 is built on it, and `docs/research/userscripts-api.md` still recommends `world:'MAIN'`, which `check-invariants.sh` forbids. |
...
| **C-06** Secrets live in the desktop store; the extension holds none | done | `host/src/secrets.ts` (`DopplerSecretProvider`, `SecretStore` with one credential per source) | `host/test/dispatcher.test.ts`: "emits llm.error no_key when no secret is available, without calling fetch", "strips a client-supplied Authorization header rather than forwarding it"; `host/test/secrets.edge.test.ts`: "trims and returns stdout on success", "resolves null when execFile reports an error", "resolves null on empty or whitespace-only stdout, without treating it as an error", "shells out to doppler with the secret name and the project/config from env, as an argument array" (the real `DopplerSecretProvider` itself, previously exercised only via `FakeSecretProvider`); `host/test/secrets.test.ts`: "loads OpenRouter and Kilo independently and reports both ready", "does not crash when only OPENROUTER_API_KEY is absent, and still loads Kilo", "does not crash when only KILO_CODE_API_KEY is absent, and still loads OpenRouter", "does not crash when neither key is available" | Doppler today behind a one-method `SecretProvider` seam, so OpenBao or an OS keychain is a swap that touches no protocol. `KILO_CODE_API_KEY` was added alongside `OPENROUTER_API_KEY`, same Doppler project/config; the two load in parallel and each source's readiness is tracked independently (`SecretStore.status(source)`), so either one being absent never crashes the host or blocks the other. |
| **C-07** Whatever holds the key does not choose the model | done | `src/runtime/runManager.ts` (`createModel` per role, now also threading `source`), `src/agent/models.ts`, `host/src/llm.ts` (`resolveUrl`, `modelsAll`), `host/src/cassette.ts` (`normalizeRequest` origin tagging), `src/host/fetch.ts` (allow-listed origins) | `src/runtime/runManager.test.ts`: "builds the leader and the follower models separately (R-11/C-07)", "threads each role's stored source through to createModel, independently of the other role"; `host/test/dispatcher.test.ts`: "fetches both catalogs in parallel and merges them under sources, with no Authorization for OpenRouter", "still returns the surviving source when the other one fails, and says which failed"; `host/test/llm.test.ts`: "attaches the Kilo key, not the OpenRouter key, for a Kilo request", "never attaches a key to an origin the host does not recognise", "reports no_key for Kilo specifically when only the OpenRouter key is loaded"; `src/host/fetch.test.ts`: "allows the Kilo AI Gateway prefix alongside OpenRouter", "still refuses an arbitrary host that merely resembles a known prefix", "never injects the OpenRouter-shaped provider block into a Kilo request" | The host proxies the request body verbatim and never rewrites `model`; the panel's two selections (model + source) are the only source. Which credential is attached is decided solely by the request's own URL origin, never by anything the client claims — proved by a direct refusal test, not just absence of a bug. |
...
3. **O-03 — what "debug userscripts live" includes. Answered by the user on 2026-09-04,
   no longer an assumption.** The answer is *the agent writes and iterates scripts*: it
   authors the code, runs it, reads the console and the error, and revises. The user was
   shown that breakpoints would need `chrome.debugger`'s `Debugger` domain — whose
   permanent infobar is a hard failure of R-02 — and chose to **drop breakpoints and keep
   R-02**. So the debug surface is console output, thrown errors, and errors mapped back
   to the author's own line and column; there is no stepping and none is planned.
   Built as `write_userscript` / `list_userscripts` Follower tools over
   `src/userscripts/authoring.ts`. `REQUIREMENTS.md` still lists O-03 as open because it
   is canonical and never edited by an agent; the answer is recorded here.
